...
let getCode = async () => {
  ...
  requestWithCaptcha(
    async (token: string) => {
      try {
        // 仓库发起验证码请求
        let result = await ownerStore.botPhoneCode({
...
const requestLoginQRCode = async () => {
  ...
  try {
    ...
    const result = await ownerStore.botGetQrcode();
...
const createBotByQRCode = async () => {
  try {
    ...
    const result = await ownerStore.botBindQrcode({
